...
class HybridModel(LanguageModule, GraphableMegatronModule):
    ...
    def _should_call_local_cudagraph(self, *args, **kwargs):
        """Return whether block-scoped inference CUDA graphs should run."""
        ...
        if (
            ...
            if inference_context.is_static_batching():
                return inference_context.is_decode_only()
            return inference_context.using_cuda_graph_this_step()
        return False

    def __call__(self, *args, **kwargs):
        # Normalize the deprecated alias before CUDA-graph dispatch. The graph manager
        # keys inference calls on ``inference_context`` and does not inspect
        # ``inference_params`` itself.
        if "inference_params" in kwargs:
            inference_params = kwargs.pop("inference_params")
            kwargs["inference_context"] = deprecate_inference_params(
                kwargs.get("inference_context"), inference_params
            )
    ...
    def create_mcore_cudagraph_manager(self, config):
        """Create the manager for block-scoped local inference graphs."""
