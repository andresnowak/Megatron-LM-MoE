...
def _add_training_args(parser):
    ...
    group.add_argument(
        ...
        "md_decoupling. 'embed' selects row normalization for input projections and "
        "column normalization for output projections. Defaults to 'flat'.",
    )
    group.add_argument(
        '--hypersphere-family-modes',
        type=str,
        nargs='*',
        default=(),
        metavar='FAMILY=MODE',
        ...
        'modes are row, col, flat, and none. Explicit overrides take precedence '
        'over --hypersphere-mode.',
    )
    group.add_argument(
        '--hypersphere-embedding-mode',
        type=str,
        default='row',
...
def _add_data_args(parser):
    ...
    group.add_argument('--goldfish-loss', action='store_true',
                       help='Enable Goldfish loss during pretraining: deterministically drop '
                            ...
                            'Train split only. Special tokens declared by the (HuggingFace-backed) '
                            'tokenizer are never dropped. Supported by pretrain_gpt.py only.')
